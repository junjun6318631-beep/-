...
def render(scale=2.0, obstacles=(), dino_y=policy.GROUND_Y, night=False,
           canvas_units=600, page_text=True):
...
class TestObserve(unittest.TestCase):
    ...
    def test_큰_선인장_여러_개를_따로_읽는다(self):
        ...
        self.assertEqual(len(obs.obstacles), 2)
        self.assertAlmostEqual(obs.obstacles[0].x, 250, delta=3)
        self.assertAlmostEqual(obs.obstacles[1].x, 420, delta=3)
        self.assertAlmostEqual(obs.obstacles[1].w, 51, delta=4)

    def test_나는_새는_떠_있는_것으로_읽는다(self):
        for y in (50, 75):
            obs, _ = self._observe([policy.Obstacle(x=350, y=y, w=46, h=40)])
            self.assertEqual(len(obs.obstacles), 1, f"y={y} 새를 못 찾음")
            found = obs.obstacles[0]
    ...
    def test_읽은_값으로_새_높이_판단이_맞는다(self):
        cfg = policy.Config()
        obs, _ = self._observe([policy.Obstacle(x=350, y=50, w=46, h=40)])
        self.assertTrue(policy.passable_on_ground(obs.obstacles[0], False, cfg),
                        "높이 나는 새는 서서 지나갈 수 있어야 한다")
        obs, _ = self._observe([policy.Obstacle(x=350, y=75, w=46, h=40)])
        self.assertFalse(policy.passable_on_ground(obs.obstacles[0], False, cfg))
        self.assertTrue(policy.passable_on_ground(obs.obstacles[0], True, cfg),
                        "가운데 높이의 새는 숙이면 지나갈 수 있어야 한다")
        obs, _ = self._observe([policy.Obstacle(x=350, y=100, w=46, h=40)])
        self.assertFalse(policy.passable_on_ground(obs.obstacles[0], True, cfg),
                         "낮게 나는 새는 숙여도 못 지나간다")

    def test_밤_모드에서도_똑같이_읽는다(self):
        cactus = policy.Obstacle(x=300, y=90, w=25, h=50)
        obs, calib = self._observe([cactus], night=True)
        self.assertTrue(calib.night)
        self.assertEqual(len(obs.obstacles), 1)
        self.assertAlmostEqual(obs.obstacles[0].x, 300, delta=3)

    def test_공룡_높이를_읽는다(self):
        frame = render(scale=2.0)
        calib = vision.calibrate(frame)
        ...
        frame = render(scale=2.0, dino_y=40)          # 점프 중
        obs = vision.observe(crop_region(frame, calib), calib)
    ...
    def test_배율이_달라도_같은_게임_좌표로_읽는다(self):
        ...
        for scale in (1.0, 2.0, 3.0):
            obs, _ = self._observe([cactus], scale=scale)
            self.assertEqual(len(obs.obstacles), 1, f"배율 {scale}")
            self.assertAlmostEqual(obs.obstacles[0].x, 380, delta=4, msg=f"배율 {scale}")


class TestSpeedEstimator(unittest.TestCase):
    def test_장애물_이동량으로_속도를_잰다(self):
        est = vision.SpeedEstimator(initial=6.0) if np is not None else None
        if est is None:
            self.skipTest("numpy 없음")
        speed = 9.0
        x = 500.0
        now = 0.0
        for _ in range(60):
            est.update([policy.Obstacle(x=x, y=90, w=25, h=50)], now)
            x -= speed
            now += 1000.0 / 60.0
        self.assertAlmostEqual(est.speed, speed, delta=0.4)

    def test_장애물이_바뀌면_튀지_않는다(self):
        if np is None:
            self.skipTest("numpy 없음")
        est = vision.SpeedEstimator(initial=9.0)
        est.update([policy.Obstacle(x=120, y=90, w=25, h=50)], 0.0)
        # 앞 장애물이 지나가고 뒤 장애물이 목표가 되면 x 가 크게 뛴다
        est.update([policy.Obstacle(x=560, y=90, w=25, h=50)], 16.7)
        self.assertAlmostEqual(est.speed, 9.0, delta=0.1)
